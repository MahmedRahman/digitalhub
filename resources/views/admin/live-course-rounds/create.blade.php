    ...
    <div class="container">
        <div class="card">
            <div class="card-header">
                <h5 class="mb-0">إضافة جولة جديدة</h5>
            </div>
            <div class="card-body">
                <form action="{{ route('admin.live-course-rounds.store') }}" method="POST">
                    @csrf
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select select2 @error('livecourse_id') is-invalid @enderror" 
                                ...
                                @foreach($livecourses as $course)
                                    <option value="{{ $course->id }}" {{ old('livecourse_id') == $course->id ? 'selected' : '' }}>
                                        {{ $course->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control @error('round_name') is-invalid @enderror" 
                                   id="round_name" name="round_name" value="{{ old('round_name') }}" required>
                            @error('round_name')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select @error('status') is-invalid @enderror" 
                                ...
                                <option value="ongoing" {{ old('status') == 'ongoing' ? 'selected' : '' }}>جاري</option>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="number" class="form-control @error('hours_count') is-invalid @enderror" 
                                   id="hours_count" name="hours_count" value="{{ old('hours_count') }}" required min="1">
                            @error('hours_count')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
